...
def main(M, N, K, in_dtype, out_dtype, only_once, trials):
    ...
    def tune(out_dtype):
        ...
        func = te.create_prim_func(
            matmul_fp16(N=N, M=M, K=K, in_dtype=in_dtype,
        ...
        with tempfile.TemporaryDirectory() as work_dir:
            ...
            sch = db.query_schedule(mod, target=target, workload_name=f"main")
            with tvm.transform.PassContext(config={"tir.use_async_copy": 1}):
                ...
                c_tvm = tvm.nd.array(np.empty((N, M)).astype(out_dtype),
                                     device=tvm.cuda(0))
                evaluator = rt_mod.time_evaluator(rt_mod.entry_name,
                                                  dev,
                                                  number=200)
...
if __name__ == "__main__":
    ...
    costs = []
    with open(f"tensorir_meta_schedule_results_{args.trials}_trials.csv",
              "w") as fout:
    ...
    for i in tqdm(range(args.begin, args.begin + args.num)):
        ...
        with open(f"tensorir_meta_schedule_results_{args.trials}_trials.csv",
            ...
            print(
                f"{M},{N},{K},{args.in_dtype},{args.acc_dtype},{cost},{tflops}",
                file=fout)
    print("Done!")
